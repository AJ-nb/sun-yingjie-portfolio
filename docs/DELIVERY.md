...
2026-09-15已成功部署第三版，当前仍仅本人访问。11项授权HTTP核对通过：模型、媒体、页面程序和下载件与本地字节一致；首页仅多出已识别的CDN挑战脚本，排除该脚本并规范化空白后与发布HTML一致。匿名请求返回403，尚未通过匿名浏览器验收。按已确认方案，仍待人物定稿后开放公众访问。
...
- 网页下载版：`deliverables/portfolio/sun-yingjie-portfolio-web.pdf`，19.443MiB；与高清原版保留相同116页、文字、书签和链接，仅优化内嵌图片。网站下载此版本。
...
网页优化版也完成116页渲染、29张联系表与12个重点区域对照；所有页面文字、内容流、书签目标和链接动作/矩形与高清原版一致。详见 `portfolio-web-qa.md`。原浏览器记录的54MB下载是优化前测试记录，当前远端文件以 `remote-delivery-verification.json` 为准。
...
- 托管服务首次拒绝包含54,192,777字节PDF的源码推送，返回对象过大，未说明精确阈值。已改用20,387,294字节网页PDF并成功部署；高清原版保留在私人GitHub。小于20MiB是本次采用的输出约束，不冒充托管服务的官方限制。
- 依赖审计仍有8项公告，当前静态网站未发现可达利用路径；临时开发/预览服务已停止。持续开发前的升级事项见 `dependency-maintenance.md`。
