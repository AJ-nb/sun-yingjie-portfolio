...
核对快照：`2026-09-14T17:09:27.018557+00:00`。范围为本项目现有来源清单、当前60份案例Markdown及本地构建；不声称覆盖整台电脑或GitHub账户中的所有项目。
...
本地构建实际选择 **163 项媒体资产，共 44,534,956 字节（42.47 MiB）**。本文“公开版”仅指这些已纳入本地构建的资产，不代表已经完成远端部署。

机器可读逐项记录：`private/sources/coverage.json`。完整SHA256、源文件与归档校验、每页PDF去向、PPTX每页和嵌入图、163项构建资产及89项未选择中间文件均记录其中。
...
`web/vite.config.ts` 的生产构建关闭自动public复制；`scripts/stage-public.mjs`从正文引用、封面、30张缩略图和3项固定资产建立选择清单。`web/dist/media-index.json` 与当前正文推导的选择集合一致，逐项文件大小与SHA256核对一致。

| 构建组成 | 项数 |
| --- | ---: |
| 案例图片与视频（按URL去重） | 130 |
| 案例缩略图 | 30 |
| 头像GLB、头像静帧、精选作品PDF | 3 |
| 合计 | 163 |

`web/public/`当前有 252 个文件，其中 89 个、24,648,123字节（23.51 MiB）未纳入构建。它们主要是精确重复的旧版页、个人资料页、替代展板图与头像审阅图。目录名称public不构成已发布证据。

精选下载PDF包含60个原作品页面：portfolio-51 p.3–50与office-fitness p.1–12。p.50总览只作为下载PDF内页存在，网站案例使用9张拆出的展板。个人介绍、原简历与证件照未作为整页放入精选PDF。该下载件不是30个网站案例的完整离线副本：它未包含网站全部商业、品牌与数字案例。

当前公开头像静帧来自新头像模型的正面渲染（`scripts/prepare_web_assets.py`记录处理方式），不是直接公开本地肖像原照；GLB与静帧属于本次网站制作资产，另列于构建清单，不计作历史来源中的新案例。
...
2份PPTX包均通过ZIP完整性检查，21个幻灯片XML和15个嵌入图均有逐项SHA256记录。个人介绍1图、市场报告14图均已目视查看；未重新渲染整份PPTX布局。PPTX、页图与嵌入图均未纳入163项构建清单。
...
- 4份本地PDF、14项本地媒体、2份PPTX、11个工程原件和66份本地数字证据的源文件均重新核对；相关私有归档与清单SHA256一致。
- 30份GitHub原件重新计算Git blob SHA-1并与记录相符，同时记录实际SHA256；这验证既有快照，不表示重新同步远端最新分支。
- 22份品牌/数字衍生资产的源与输出SHA256均一致；9个内嵌JP2与公开WebP映射存在。
- 163项媒体在public源目录与dist构建中的文件字节和SHA256一致，选择集合与当前Markdown相符。

本次检查问题数：**0**。所有声明针对上述快照；主任务后续若更改案例、资产或重新构建，应以最新构建重新核对本报告的数量和摘要。
...
本报告只记录来源、去向和文件完整性，没有重新执行各数字工具的功能测试，也不把文件校验视为医疗、材料、飞行、量产或商业绩效验证。远端同步、构建体验与最终发布由主任务处理。
