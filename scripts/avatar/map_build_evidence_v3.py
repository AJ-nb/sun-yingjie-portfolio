...
lines=(root/'scripts/avatar/build_avatar_v3.py').read_text(encoding='utf8').splitlines()
groups=[]
...
bind(['globe.parent','iris.parent','ring.parent','pupil.parent','obj.parent'],[ref('Object','parent')],'Set object hierarchy. Eye-child vertices are already authored relative to the eye root pivot; the avatar root is identity.')
...
for p in ['frame_start','frame_end']:
    bind(['scene.'+p+'='],[ref('Scene',p)],'Set playback range for an 350-frame portrait transition at 24 fps.')
...
(root/'scripts/avatar/operation-bindings-v3.json').write_text(json.dumps(groups,indent=2),encoding='utf8')
