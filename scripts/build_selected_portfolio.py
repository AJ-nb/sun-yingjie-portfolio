...
INK, PAPER, SAGE, MUTED, LINE = '#131613', '#ffffff', '#675973', '#61665f', '#d9ddd1'
W, H = 960, 600
...
def read_cases(lang="zh"):
    result = {}
    for path in (ROOT / 'web/src/content/works').glob(f'*.{lang}.md'):
...
class Edition:
    def __init__(self, filename):
        register_fonts()
        self.archive_image_profile = os.environ.get('PORTFOLIO_PDF_IMAGE_PROFILE') == 'archive-web'
    ...
    def para(self, s, x, top, width, size=11, leading=None, color=INK, bold=False, floor=42):
        use_latin = getattr(self,'lang','zh')=='en' and not re.search(r'[\u3400-\u9fff]', s)
        font=('BodyBold' if bold else 'Body') if use_latin else ('CNB' if bold else 'CN')
        lines=[]
    ...
    def image(self, public_path, x, y, w, h):
        ...
        # The complete archive may contain every supplied board. Its pages show
        # one or two boards at print-like scale, so carrying the full source
        # PNG for each board only bloats a download without adding page detail.
        # This profile is used solely by the full archive; selected editions
        # retain their higher-resolution image treatment.
        if self.archive_image_profile:
            im.thumbnail((760, 950), Image.Resampling.LANCZOS)
        else:
            im.thumbnail((2400, 2000), Image.Resampling.LANCZOS)
        buffer = io.BytesIO()
        if im.mode == 'RGBA': im.save(buffer, 'PNG')
        else: im.save(buffer, 'JPEG', quality=55 if self.archive_image_profile else 91, subsampling=2 if self.archive_image_profile else 0)
