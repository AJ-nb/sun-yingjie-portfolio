...
assert(pairs.size >= 30, `Expected all 30 cases, got ${pairs.size}`)
const legacy = JSON.parse(await readFile(path.join(root, 'private/sources/legacy-manifest.json'), 'utf8'))
const oldImages = legacy.filter(item => item.publicPath)
for (const item of oldImages) assert(media.has(item.publicPath), `Legacy image missing a case: ${item.publicPath}`)
console.log(`PASS: ${pairs.size} bilingual cases, ${media.size} media references, all ${oldImages.length} legacy work images covered.`)
