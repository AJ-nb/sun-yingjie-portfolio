...
const story = require(path.join(root, 'web/src/data/portfolioV6.json'))
const base = new URL(process.env.BASE_URL || process.env.PORTFOLIO_QA_URL || 'http://127.0.0.1:5176').href.replace(/\/?$/, '/')
const output = path.resolve(root, process.env.PORTFOLIO_QA_OUTPUT || 'design/qa-v6')
...
async function main() {
  ...
  for (const width of [320, 390, 768, 1440]) {
    ...
    await page.locator('.v5-hero').waitFor()
    await check(`responsive layout ${width}px`, async () => {
      ...
      const representatives = await page.locator('[data-representative]').evaluateAll(nodes => nodes.map(node => node.dataset.representative))
      const careers = await page.locator('[data-career-chapter]').evaluateAll(nodes => nodes.map(node => node.dataset.careerChapter))
      const openSource = await page.locator('[data-open-source]').evaluateAll(nodes => nodes.map(node => node.dataset.openSource))
      assert.deepEqual(representatives, story.representatives.map(item => item.id))
      assert.deepEqual(careers, story.careerChapters.map(item => item.id))
      assert.deepEqual(openSource, story.openSource.map(item => item.slug))
      assert.equal(await page.locator('[data-download="selected"]').count() > 0, true)
      assert.equal(await page.locator('video').count(), 0, 'Reduced motion must render static posters without video elements')
      const sectionOrder = await page.locator('main section[id]').evaluateAll(nodes => nodes.map(node => node.id))
      assert.ok(sectionOrder.indexOf('about') < sectionOrder.indexOf('selected'), JSON.stringify(sectionOrder))
      assert.ok(sectionOrder.indexOf('selected') < sectionOrder.indexOf('career'), JSON.stringify(sectionOrder))
      assert.ok(sectionOrder.indexOf('career') < sectionOrder.indexOf('open-source'), JSON.stringify(sectionOrder))
      assert.ok(sectionOrder.indexOf('open-source') < sectionOrder.indexOf('works'), JSON.stringify(sectionOrder))
      assert.ok(sectionOrder.indexOf('works') < sectionOrder.indexOf('profile-details'), JSON.stringify(sectionOrder))
      assert.ok(sectionOrder.indexOf('profile-details') < sectionOrder.indexOf('process'), JSON.stringify(sectionOrder))
      if (width <= 390) {
        const targets = await page.locator('.mobile-dock a').evaluateAll(nodes => nodes.map(node => ({ width: node.getBoundingClientRect().width, height: node.getBoundingClientRect().height })))
        assert.equal(targets.length, 4)
        assert.ok(targets.every(target => target.width >= 44 && target.height >= 44), JSON.stringify(targets))
      }
      return { overflow, representatives, careers, openSource, sectionOrder }
    }, page)
  ...
  await check('archive controls, case route, history and lightbox', async () => {
    ...
    const entry = page.locator('[data-representative] a[href^="#/work/"]').first()
    ...
    await page.locator('[data-representative]').first().waitFor()
