...
它记录一次独立的 AI 视频工作流研究：怎样把镜头参考、一个新剧情和少量角色输入，变成可检查的镜头、时间、动作与画面约束。

成片是作者提供的 1080 × 1440、22.08 秒视频。页面聚焦工作流设计、视觉控制与人工判断。

## 本人职责与输入边界

本人负责把剧情改写为分镜脚本，定义镜头运动和画面不变量，安排 Blender 白模预演，并组织供 Seedance 2.5 使用的角色图、场景图和提示词。过程参考了一段电影场景的运镜语言；该参考片不在本页发布，也不被归为本项目素材。

过程记录显示：两张主角角色图用于扩展人物、提取风格线索与撰写视频提示词；22 秒成片包含 11 个分镜单元和 20 余个角色；“单次生成”是该次流程的制作记录。
...
这套方法可迁移到产品和空间表达：以产品形态或使用动作作为视觉不变量，以分镜定义用户任务，再用白模检查相机、接触和尺度关系。它是一套生成前的设计判断框架。

### 从案例到方法库

将公开参考文档整理为意图、视觉一致性、时间结构、物理逻辑、镜头与评估六个控制层，再以八项方法拆解连接产品、CMF、空间和叙事。文档分析与个人流程应用分别记录。
