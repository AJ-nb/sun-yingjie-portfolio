...
import { BrandWorkbench } from './BrandWorkbench'
import { PORTFOLIO_FILTERS, portfolioDiscipline } from './portfolioTaxonomy'
...
export default function CasePage({ work, lang, reduced, openPhoto, back, visit }: { work: WorkDoc | null; lang: Lang; reduced: boolean; openPhoto: (photos: Photo[], index: number) => void; back: () => void; visit: (slug: string) => void }) {
  ...
  return <motion.main className="case-page" ref={main} tabIndex={-1} initial={reduced ? false : { opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: reduced ? 0 : .22, ease: [.22, 1, .36, 1] }}>
    <div className="case-toolbar"><button onClick={back}>← {t.back}</button><span>{PORTFOLIO_FILTERS[lang][portfolioDiscipline(work.slug)]}</span></div>
    ...
    <BrandWorkbench slug={work.slug} lang={lang} openPhoto={openPhoto}/>
