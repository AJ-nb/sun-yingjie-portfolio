...
export default function VideoReferenceIndex({lang}:{lang:Lang}) {
  ...
  return <section className="reference-catalog" id="reference-catalog">
    <div className="methods-section-title"><p>{lang==='zh'?'研究索引 / 参考资料':'Research index / reference material'}</p><h2>{lang==='zh'?'完整索引，按问题查阅。':'A complete index, organized around questions.'}</h2><p>{lang==='zh'?`${index.entries.length} 条固定版本参考记录，按分析关注点分类；观看与个人应用状态在深入解析中分别标记。`:`${index.entries.length} pinned reference records grouped by research focus. Viewing and personal application status are recorded separately in the deeper analyses.`}</p></div>
    ...
    <div className="reference-grid">{shown.map(entry=>{const tweet=/\/status\/(\d+)/.exec(entry.sourceUrl)?.[1];return <article key={entry.id} id={'reference-'+entry.id}>
      ...
      <details><summary>{lang==='zh'?'参考与资料说明':'Reference and source notes'}</summary><p>{lang==='zh'?'资料状态：文档索引。内容依据原始索引条目整理。':'Source status: document index. This entry is organized from the original index record.'}</p><p>{entry.creator} · <a href={entry.sourceUrl} target="_blank" rel="noreferrer">{lang==='zh'?'来源':'Source'}</a> · <a href={entry.recordUrl} target="_blank" rel="noreferrer">GoodCase</a></p>
