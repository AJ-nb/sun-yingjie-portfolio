...
export default function MediaViewer({ photos, index, onClose, onIndex, lang, reduced }: {
  ...
  return <Lightbox open index={index} close={onClose} plugins={[Zoom, Captions, Counter]}
    slides={photos.map(photo => ({ src: asset(photo.src), alt: photo.alt, title: photo.alt, description: photo.src.startsWith('/media/v7/') ? (lang === 'zh' ? '设计解析与应用图' : 'Design analysis and application') : mediaLabel(photo.src, lang) }))}
