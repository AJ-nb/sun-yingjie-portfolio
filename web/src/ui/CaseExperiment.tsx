...
function LogoChecks({ image, lang }: { image: string; lang: ExperimentLang }) {
  ...
  return <div className={`ce-logo-checks${inverse ? ' is-inverse' : ''}`}>
    <div className="ce-demo-heading"><span>{text(lang, '原标志局部 / 屏幕尺寸检查', 'ORIGINAL MARK CROP / SCREEN SIZE CHECK')}</span><button type="button" aria-pressed={inverse} onClick={() => setInverse(value => !value)}>{text(lang, '黑白反转', 'Invert colours')} <span aria-hidden="true">◐</span></button></div>
    <div className="ce-logo-sizes">{[16, 32, 64, 128].map(size => <figure key={size}><div className="ce-mark-crop" style={{ width: size, height: size }}><img src={asset(image)} alt={text(lang, `${size}像素宽的标志局部`, `Mark crop at ${size} pixels wide`)} loading="lazy" decoding="async" /></div><figcaption>{size}px</figcaption></figure>)}</div>
    <p>{text(lang, '显示宽度为CSS像素。使用同一原图的标志区域裁切，未重画；小尺寸和实物工艺仍需单独验证。', 'Widths are CSS pixels. Every sample crops the same original mark without redrawing. Small-size use and physical production need separate validation.')}</p>
  </div>
}
