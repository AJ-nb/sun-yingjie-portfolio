...
export function StudioProfile({ lang, visit, openCapabilities, setOpenCapabilities }: Props & { openCapabilities: string[]; setOpenCapabilities: Dispatch<SetStateAction<string[]>> }) {
  ...
  return <section id="about" className="studio-profile" aria-label={lang === 'zh' ? '关于孙英杰' : 'About Yingjie Sun'} data-motion-anchor>
    <div className="profile-heading"><div><span className="studio-eyebrow">{lang === 'zh' ? '关于我' : 'About me'}</span><h2 id="profile-title">{lang === 'zh' ? '孙英杰' : 'Yingjie Sun'}<span>{lang === 'zh' ? 'Yingjie Sun' : '孙英杰'}</span></h2></div><div>{p.lead}</div></div>
    <div className="profile-lead"><div className="profile-sidebar"><img className="profile-photo" src={asset('/media/v5/portrait.png')} alt={lang === 'zh' ? '孙英杰个人肖像' : 'Portrait of Yingjie Sun'} loading="lazy" onError={event => { event.currentTarget.hidden = true }}/><dl className="profile-facts">{p.facts.map(fact => <div key={fact.label}><dt>{fact.label}</dt><dd>{fact.value}</dd></div>)}</dl></div><div className="profile-narrative">{p.narrative.map(paragraph => <p key={paragraph}>{paragraph}</p>)}</div></div>
    ...
    <div className="profile-section"><div className="profile-section-title"><span>04</span><h3>{sections[3]}</h3><p>{lang === 'zh' ? '网页适合探索，文档适合认真阅读与交流。' : 'Explore on the web; keep the documents for a closer read.'}</p></div><div className="profile-downloads">{[
      { path: 'selected-portfolio.pdf', title: lang === 'zh' ? '精选作品集' : 'Selected portfolio', note: lang === 'zh' ? '32 页 / 六个精选案例' : '32 pages / Six selected cases' },
      { path: 'full-portfolio.pdf', title: lang === 'zh' ? '完整作品集' : 'Complete portfolio', note: lang === 'zh' ? '扩展案例 / 完整项目档案' : 'Extended cases / Project archive' },
...
export function StudioContact({ lang, reduced }: { lang: Lang; reduced: boolean }) {
