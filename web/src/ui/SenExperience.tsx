...
export function SenIntroduction({ lang, reduced, portrait, anchor }: { lang: Lang; reduced: boolean; portrait: ReactNode; anchor: (event: MouseEvent<HTMLAnchorElement>) => void }) {
  ...
  const blur = useTransform(scrollYProgress, [0, .5], ['blur(0px)', 'blur(16px)'])
  const titleY = useTransform(scrollYProgress, [0, 1], [0, -96])
  const bodyY = useTransform(scrollYProgress, [0, 1], [0, -52])
  const tracking = useTransform(scrollYProgress, [0, 1], ['.01em', lang === 'zh' ? '.2em' : '.42em'])
  ...
  return <div className="intro-scene">
    ...
    <section className="sen-hero" aria-labelledby="hero-name">
      <motion.div className="sen-hero-copy" style={reduced ? undefined : { opacity, filter: blur }}>
        <motion.h1 id="hero-name" style={reduced ? undefined : { y: titleY, letterSpacing: tracking }}>{profile.name[lang]}</motion.h1>
      ...
      <motion.div className="wk-track" style={vertical ? undefined : { x }}>
        {CHAPTERS.map((chapter, index) => {
          ...
          return <article className="wk-card" key={chapter.id} aria-label={chapter.title[lang]}>
            ...
            {cover && <a className={`wk-cover wk-cover-${chapter.id}`} href={`#/work/${cover.slug}`} tabIndex={tabIndex} onClick={event => { event.preventDefault(); visit(cover.slug) }}><img src={asset(cover.cover)} alt={cover.title} loading="lazy" /></a>}
            <div className="wk-card-body"><p className="wk-chapter-note">{chapter.note[lang]}</p><ul className="wk-list">{picks.map(work => <li key={work.slug}><a href={`#/work/${work.slug}`} tabIndex={tabIndex} onClick={event => { event.preventDefault(); visit(work.slug) }}><span>{work.title}</span><span aria-hidden="true">↗</span></a></li>)}</ul><a className="wk-catalogue-link" href="#works" tabIndex={tabIndex} onClick={catalogue}>{lang === 'zh' ? '完整目录与更多项目' : 'Full index & more projects'} <span aria-hidden="true">↓</span></a></div>
          </article>
        })}
      </motion.div>
