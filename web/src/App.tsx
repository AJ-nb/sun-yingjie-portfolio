...
import { StudioIntroduction, StudioSelected } from './ui/Studio'
...
export default function App() {
  ...
  return <>
    ...
    <SiteHeader lang={lang} isCase={displayedIsCase} reduced={reduced} systemReduced={systemReduced} anchor={homeAnchor} toggleMotion={toggleMotion} toggleLanguage={toggleLanguage} />
    ...
    {isCase ? <motion.div key={hash} className="case-transition">
      ...
      <section className="work-index" id="works" aria-labelledby="index-title" tabIndex={-1} data-motion-anchor>
        <div className="index-heading"><div><span className="studio-eyebrow">{lang === 'zh' ? '完整档案' : 'Full archive'}</span><h2 id="index-title">{t.catalog}</h2><p>{lang === 'zh' ? '每一个项目，都是一次具体的提问。按领域筛选，或切换为列表，找到你想深入了解的部分。' : 'Every project begins with a particular question. Filter by discipline or switch to the list to find your way into the work.'}</p></div><span>{String(works.length).padStart(2, '0')}<small>{t.count}</small></span></div>
