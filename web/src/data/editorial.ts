...
export function mediaLabel(src: string, lang: Lang): string {
  const kind = src.includes('/media/v7/') ? 'analysis' : src.includes('/ai-v3/') ? 'concept' : src.endsWith('.svg') ? 'diagram' : src.includes('/documents/') || src.includes('/psd-') ? 'board' : /\/digital\/|refinement-brand|refinement-digital/.test(src) ? 'screen' : 'original'
  const labels = { analysis: ['设计解析与应用图', 'Design analysis and applications'], concept: ['概念表现图', 'Concept illustration'], diagram: ['设计说明图', 'Design diagram'], board: ['原始项目展板', 'Original project board'], screen: ['界面与流程记录', 'Interface / workflow record'], original: ['原始项目图', 'Original project visual'] }
